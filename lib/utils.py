...
def evaluate(base_net, loader, img_size, pred_net, status=None, vis=False, logger=None,
    ...
    :param pred_net: network for prediction
    ...
    assert pred_net is not None, 'ERROR: prediction net is NOT specified!'

    device = next(base_net.parameters()).device

    nets = [base_net, pred_net]
    previous_states = []
    for net in nets:
